...
public class PlayerTerritorySpawn : ChessboardGenerator, IMainBoardInfo
{
    private Points[] armyEdgedPoints;
    private ChessboardTile[] armyEdgedobjects;

     private List<GameObject> spawnedObjects = new List<GameObject>();
    private List<ChessboardTile> connections = new List<ChessboardTile>();
    ...
    [SerializeField] private CentralBoardGenerator boardGenerator;
        ...
        switch (teamNumber.Value)
        {
            case 1:
                ...
                if (gameManager.GetMainBoard().TileEdgedPoints[3].X > armyEdgedobjects[0].CurrentTilePoint().X)
                {
                    ...
                    foreach (ChessboardTile connection in connections)
                    {
        ...
        if (armyEdgedPoints.Length > 0)
        {
            for (int i = 0; i < armyEdgedPoints.Length; i++)
            {
                if (setupTiles.ContainsKey(armyEdgedPoints[i]))
                    armyEdgedobjects[i] = setupTiles[armyEdgedPoints[i]].GetComponent<ChessboardTile>();
    ...
    private void PositionAllPiecesServerRpc()
    {
        ...
        for (int i = 0; i < spawnedObjects.Count; i++)
        {
            ...
            if (setupTiles.ContainsKey(setPoint))
            {
                ...
                ChessboardTile boardPiece = setupTiles[setPoint].GetComponent<ChessboardTile>();
    ...
    #region Public
    public void CreatePieceList(ChessboardTile connection) => connections.Add(connection);
    ...
    public override void OnNetworkSpawn()
    {
        ...
        boardGenerator = CentralBoardGenerator.Instance;
    }
    ...
    public void SetChessboardTemplate(ChessboardTemplate chessTemp)
    {
        ...
        armyEdgedobjects = new ChessboardTile[armyEdgedPoints.Length];
    }
    public void SetupVariables(int _teamNumber, CentralBoardGenerator _chessGen, GameManager _gameManager)
    {
