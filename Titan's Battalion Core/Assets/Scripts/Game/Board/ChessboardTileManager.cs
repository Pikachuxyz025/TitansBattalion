...
public class ChessboardTileManager : NetworkBehaviour
{
    public static ChessboardTileManager instance;
    ...
    public ChessboardTile GetChesspieceConnection(Points currentPoint)
    {
        // check if tile is here
        if (!Tiles.ContainsKey(currentPoint))
            return null;
        return Tiles[currentPoint].GetComponent<ChessboardTile>();
    }
    ...
    public bool IsOccupied(Points currentPoint)
    {
        ...
        ChessboardTile connection = GetChesspieceConnection(currentPoint);
    ...
    public Vector3 GetNewPiecePosition(Points currentPoint)
    {
        if (!Tiles.ContainsKey(currentPoint))
            return Vector3.zero;
        Vector3 newPosition = Tiles[currentPoint].GetComponent<ChessboardTile>().pieceSpawnPoint.transform.position;
        return newPosition;
    }
    ...
    public void ResetTiles()
    {
        foreach (GameObject conn in Tiles.Values)
            conn.GetComponent<ChessboardTile>().piecesThatHaveUsInCheck.Clear();
    ...
    public void PositionSinglePiece(Chesspiece cp, ChessboardTile dp)
    {
    ...
    {
        ChessboardTile dp = GetChesspieceConnection(pd);
        cp.SetNewCurrentPoints(dp.GridX.Value, dp.GridY.Value);
        Vector3 pos = dp.pieceSpawnPoint.transform.position;
