...
public class ChessGen_Test : ChessGenerator, IMainBoardInfo
{
    ...
    [ClientRpc]
    public void SetupTerritoryBoardClientRpc()
    {
        Button rButton = GameUIManager.instance._readyButton.GetComponent<Button>();
        readyButton.onClick.AddListener(SetupTerritory);
    }
    ...
    public void SetupTerro()
    {
        if (IsServer)
            readyButton.onClick.AddListener(SetupTerritory);
    }

    public void SetupTerritoryBoard()
    {
        readyButton.onClick.AddListener(SetupTerritory);
    }
    ...
    {
        if (armyBoardList.Count >= uiManager.armySelection.value && uiManager.armySelection.value != 0)
        {
            //Debug.Log("There shouldn't be an error " + armyBoardList.Count + " : " + uiManager.armySelection.value);

            SetChessIdServerRpc(uiManager.armySelection.value - 1);
        }
        else
            return;
        if (IsOwner)
    ...
    void AssignRookToKing(King king)
    {
        ...
        foreach (GameObject _object in spawnedObject)
        {
            if (_object.GetComponent<Chesspiece>() is Rook)
                king.rooks.Add(_object.GetComponent<Rook>());
        }
    }
